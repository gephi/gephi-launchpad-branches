...
public class MultiKnobSliderAdditionalUI {
  
  MultiKnobSlider  mSlider;
  BasicSliderUI ui;
  ...
  public MultiKnobSliderAdditionalUI(BasicSliderUI ui)   {
    this.ui      = ui;
  }

  
  public void installUI(JComponent c)   {
    mSlider    = (MultiKnobSlider)c;
    ...
    isDragging = false;
    trackListener = new MultiKnobSliderAdditionalUI.TrackListener(mSlider);
  ...
  protected void calculateThumbsSize() {
    Dimension size = ((MultiKnobSliderAdditional)ui).getThumbSize();
    for (int i=0; i<thumbNum; i++) {
  ...
  protected void calculateThumbsLocation() {
    for (int i=0; i<thumbNum; i++) {
      ...
      if ( mSlider.getOrientation() == JSlider.HORIZONTAL ) {
        ...
        int valuePosition = ((MultiKnobSliderAdditional)ui).xPositionForValue(value);
        thumbRects[i].x = valuePosition - (thumbRects[i].width / 2);
        thumbRects[i].y = trackRect.y;
        
      } else {
        int valuePosition = ((MultiKnobSliderAdditional)ui).yPositionForValue(mSlider.getValueAt(i));     // need
  ...
  public Rectangle getTrackRect() {
    return ((MultiKnobSliderAdditional)ui).getTrackRect();
  }
  ...
  public class TrackListener extends MouseInputAdapter {
    ...
    protected MultiKnobSlider   slider;
    protected Rectangle trackRect;
    
    TrackListener(MultiKnobSlider slider) {
      this.slider = slider;
    }
